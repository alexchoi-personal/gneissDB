...
async fn main() {
    ...
    println!("\nSlow ops (>100µs): {} ({:.1}%)", slow_count, slow_count as f64 / count as f64 * 100.0);
    println!("Very slow ops (>1ms): {} ({:.1}%)", very_slow_count, very_slow_count as f64 / count as f64 * 100.0);
    ...
    println!("RocksDB Total: {:.3}ms = {:.0} ops/sec\n", 
    ...
    println!("\n=== Comparison ===");
    println!("Throughput: rocksdb-rs is {:.2}x slower", 
        total.as_secs_f64() / rocks_total.as_secs_f64());
    println!("p50 latency: rocksdb-rs is {:.2}x higher",
